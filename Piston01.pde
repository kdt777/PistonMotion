...
void draw() {
  background(192);      // sets the background colour (255=white)
  
  float alph = radians(alpha);
  float cosA = cos(alph);
  
  //circle(cLine, sHght, 50);          // crankshaft
  float bigEndX = cLine + scale*sin(alph);
  float bigEndY = sHght - scale*cos(alph);
  ...
  textSize(14);
  text("press P to Pause/Play", 400, 400);
  text("press + or - adv/ret 1 deg", 400, 420);
